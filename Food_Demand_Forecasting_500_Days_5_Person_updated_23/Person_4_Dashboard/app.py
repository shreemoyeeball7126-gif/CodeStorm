...
# Tell the browser itself to never render this page in dark mode, at the
# earliest possible point (before any other styling loads).
st.markdown(
...
/* Selectbox — these are Streamlit's own stable internal class names for
   this exact component (verified directly against the installed Streamlit
   build, not guessed), so they work regardless of whether the theme config
   loads or what the viewer's OS/browser dark-mode setting is:
     e1fp86qc0 = control box (the visible closed pill)
     e1fp86qc1 = the value text inside it
     e1fp86qc2 = the dropdown arrow icon
     e1fp86qc4 = the open dropdown/popover background
     e1fp86qc5 = the option list
     e1fp86qc7 = each option row's text
*/
.e1fp86qc0 {
...
.e1fp86qc7:hover, .e1fp86qc7[data-hovered] {
    background-color: #F0EEE3 !important;
}

/* Belt-and-braces fallback in case a future Streamlit patch changes those
   class names — same intent, matched structurally instead. */
